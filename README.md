...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-rasmus-hutt/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..yRBF2b_5xEGipXFA-UXprkdL6l5U0riXvkprBhpGG1VyClGLLI8lA3pYBsd5uEK72S1i8p4ijcdUkbPc6dSiaja4i0YXB2LM5jYhalJlPv0mLoLfbFUyEj7ApdvSD4AaQxLct8QCuJb36vXDzYxDySB5uYy-U3ddUbEILtvnAyWMGCvI1d3mxO18rTJx_yhTFo6fGoZ9h6kTkqzKZQ8PFXteK9HbthrqQsuIeKkg6zxwSTKr-r24ZivfueqOfJcAigWX9ZUnZYSK1wrfZhxIDNxiI36cMO2_GTICyUd2B1Rf-NfbrfHKRe8HyauMlqEyfFZDVXebefg8BYUaEvcOFQ
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-rasmus-hutt
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-rasmus-hutt auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-rasmus-hutt apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-rasmus-hutt`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-rasmus-hutt apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-rasmus-hutt apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-rasmus-hutt apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-rasmus-hutt.k8s.vrfgstest.org/
  - host: "vrf-rasmus-hutt.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-rasmus-hutt.k8s.vrfgstest.org/](https://vrf-rasmus-hutt.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-rasmus-hutt get ingress
