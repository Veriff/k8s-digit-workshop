...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-sten9911/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..XvbVVbK7LBJYR3gtM537Sg2e_ZpbxDXmerwLn-GXftUwqaX4McXfl4tpQkRllZpdUqhqYwaIBVkahvzqUr1104Muwwn3mKimtN7dWx7iHp94dT75NKsqoYxJmEqWxboWs9Mo2tZO75RRnqbRZSauDZicfco3mix6JoUfrVqFDic4znWJHUNpgp4Jij3AGCYyT_RSoXtUEgH6tBeJGnFqcx1qSjTLGNo-vOaGHpMx3fWUtEhmb86qPXOZ8eTLwaKAZk0MXfAZ1bmX6M9W7ZJcpWNSnEpiP2lRE8_XPpwyQjC7qxl28w8YSiXy1C-NUchsJP9itveJoTMAkK4_z8MsdQ
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-sten9911
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-sten9911 auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-sten9911 apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-sten9911`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-sten9911 get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-sten9911 get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-sten9911 describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-sten9911 delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-sten9911 delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-sten9911 apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-sten9911 get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-sten9911 get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-sten9911 get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-sten9911 get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-sten9911 get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-sten9911 exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-sten9911 exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-sten9911 logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-sten9911 logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-sten9911 get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-sten9911 delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-sten9911 apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-sten9911 get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-sten9911 exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-sten9911 apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-sten9911.k8s.vrfgstest.org/
  - host: "vrf-sten9911.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-sten9911.k8s.vrfgstest.org/](https://vrf-sten9911.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-sten9911 get ingress
