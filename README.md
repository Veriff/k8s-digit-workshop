...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/siim-foo/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..Q6Vg1vSh72wj4eluf-mcbzQCMPe6jZF_eD9suDdXucH-ULwzkjOrY33HjQ9-L_V3e2f75JJy-jfi3X14qhxntm8gAtMoDhmMqr7JeBSc6xbb4qw7lppAh0hqhVVd77yvzxesIKV6Sq-T2ueyslxtgZAmXz-eabEoKW8bWcy17PHpBZZhY1ELGx7z3F5K5Zm55cyVNdPQZKMbw8lTvdXFZtRSVqOduM2G-lcsSIvNyI8MoPM--CkBmf-BUtfFcZ_XLQqFAFhbbc3hwtId2FWW5FomlaV1moRBQXySHRJCofcXk-MxuNpZsrVaa7zJhnfCsDx-19I2kZ7GeBPErUlGUw
...
```sh
kubectl --context=k8s-digit-workshop -n siim-foo auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n siim-foo apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=siim-foo`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=siim-foo get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=siim-foo get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=siim-foo describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=siim-foo delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=siim-foo delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n siim-foo apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=siim-foo get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=siim-foo get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=siim-foo get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=siim-foo get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=siim-foo get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=siim-foo exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=siim-foo exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=siim-foo logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=siim-foo logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=siim-foo get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=siim-foo delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n siim-foo apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=siim-foo get services
...
> `kubectl --context=k8s-digit-workshop -n siim-foo apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://siim-foo.digit.vrfgstest.org/
  - host: "siim-foo.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://siim-foo.digit.vrfgstest.org/](https://siim-foo.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=siim-foo get ingress
