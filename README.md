...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/digit-test-kk/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..nEjAJTCDCeQMXO9_4yQZAynu5KJDNAsSCRJMWFgEI1hXNaY-fnNStWQAgCKJANt_8Af3jFRo0Sn8T59wtfKSZQyOROehfNzjDFPEj1lrn8gAYLzTEr9jLEGRJf8V-xIyjcfOeLQF3UN7MrhbwmA3W1SoHFcJYa8-NSnoMuSSh0O_jrENvFYahpkIzcaVTChHZ97k8IdQlAS-Olc_9DijJlYbdLzRNVS3sygYc2CBID8yM_h9XTKGTiiUUugyb3Zv2rxcVScjxuzJBEzYaSkBzgG88vjmZWhFShAZ469NaOSf9QsGh1cVT1KI4o4IcH1a7LKtRdOdnAX-zTDeUvM_xA
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n digit-test-kk auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-digit-workshop -n digit-test-kk apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=digit-test-kk`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=digit-test-kk describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=digit-test-kk delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=digit-test-kk delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n digit-test-kk apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=digit-test-kk get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=digit-test-kk exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=digit-test-kk exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=digit-test-kk logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=digit-test-kk logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=digit-test-kk get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=digit-test-kk delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n digit-test-kk apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=digit-test-kk get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=digit-test-kk exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n digit-test-kk apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://digit-test-kk.digit.vrfgstest.org/
  - host: "digit-test-kk.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://digit-test-kk.digit.vrfgstest.org/](https://digit-test-kk.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=digit-test-kk get ingress
