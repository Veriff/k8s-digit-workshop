...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/test-xx12/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..sNAHwkg5QX1PGqhvVkSbWwGFzIEVYF-osyBgfPqWrcwadf93OeYNyhXqCsPovnr-6LEnOmI3c9H-oue1rUYmixrHdWGmGWpRjODh5TFzz-ohJvuEEOJz7Uy_6qEeDqr6ZuM3ZEeX3uy95FuDX1MWZy0YYuK8srWdjcA14EXdAx5_0k8p5Cudxl3o1XTvZ2J5utl5KRYwiIe-uYnPfwwuJWwFR6I7noJjQn0-VHxea-EotamDHQH7w-rbNUxmAjhLfxI8lX41a3NqQSy41KY7kgCI2_9zsrXrFFWxfrG0_bGbYFnyVNUJgvz8vwi9l-NJdjq7hECwTUU0J8nGEYJmgw
...
```sh
kubectl --context=k8s-digit-workshop -n test-xx12 auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n test-xx12 apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=test-xx12`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=test-xx12 get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=test-xx12 get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=test-xx12 describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=test-xx12 delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=test-xx12 delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n test-xx12 apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=test-xx12 get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=test-xx12 get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=test-xx12 get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=test-xx12 get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=test-xx12 get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=test-xx12 exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=test-xx12 exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=test-xx12 logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=test-xx12 logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=test-xx12 get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=test-xx12 delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n test-xx12 apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=test-xx12 get services
...
> `kubectl --context=k8s-digit-workshop -n test-xx12 apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://test-xx12.digit.vrfgstest.org/
  - host: "test-xx12.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
```sh
# get list of services
kubectl --context=k8s-digit-workshop -n=test-xx12 get ingress
```