...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/circleci-test/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..rFe9szJ0Aj3QAvOYYAkdxEufLMmWocFvmef5Lh1MidTXbvyojNap7oID4-TiMrJ1JrN7W0v96R_cKpYJGzyfAUoRtYOPf_HL5fABOLLGQt6PLwFHL5vd7f3ZCt_-L1S8vEVEPcvfXfUTeC9Q0QFyujSbdIoG0-dnYVV9gKBpZZf3JZosjDpA_zUMSJS_WgNHEvpi5dLk41UCYSW7j6iKPHr6T70YG_jls13EOHz9NIkCfSAofeSqQvBWftirehz88tSPcmaJTOfrSDXalOwG8i-4wHy2-aLDM2X-cVzQb6aayPrtV36-HnoNAiqsQsiFXa0JrqWcQ_DUKaRrGJY8_A
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n circleci-test auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n circleci-test apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=circleci-test`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=circleci-test get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=circleci-test get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=circleci-test describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=circleci-test delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=circleci-test delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n circleci-test apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=circleci-test get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=circleci-test get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=circleci-test get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=circleci-test get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=circleci-test get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=circleci-test exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=circleci-test exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=circleci-test logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=circleci-test logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=circleci-test get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=circleci-test delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n circleci-test apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=circleci-test get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=circleci-test exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n circleci-test apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://circleci-test.digit.vrfgstest.org/
  - host: "circleci-test.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://circleci-test.digit.vrfgstest.org/](https://circleci-test.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=circleci-test get ingress
