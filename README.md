...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/switzs/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..iE5aJxM20CkwHaEi2aafk2kGW4bNxYryda_Q6YPUr-bOepPLkEN_tfRTNjf_7Ivw6PkO7m6sbWgHzYhh7DOOkzRONtBWh53yjl8PMQyeG7nJpW4Pup9kwQrkNJdJyfvBk8wWcCd7wDwjz6GFQeJdNVC9wTOjt3s6G0KPFUvAWRN_OfqIrx8ZLhHtdZprHtJkuhfcKgMyAsserJi1ClS6nRLjKIwhPacBmdSy3-2waIbm1wfkJynTj8fpOZj9Ya1hLQL9l1PJfyGZ_Mn0qSV-L-NtRaZty2QmRTE7Q3jiEtPKcq5SQg2Y6c7rgRTzOEVo_gs75D2rx5x9dt6xh69iQg
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n switzs auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-digit-workshop -n switzs apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=switzs`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=switzs get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=switzs get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=switzs describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=switzs delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=switzs delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n switzs apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=switzs get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=switzs get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=switzs get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=switzs get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=switzs get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=switzs exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=switzs exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=switzs logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=switzs logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=switzs get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=switzs delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n switzs apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=switzs get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=switzs exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n switzs apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://switzs.digit.vrfgstest.org/
  - host: "switzs.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://switzs.digit.vrfgstest.org/](https://switzs.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=switzs get ingress
