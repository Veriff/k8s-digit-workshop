...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-laura-eliise/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..Bzx2OWAgs3y2htze3P2I-AkikuKtjpUUNWlzZN4IRaUO0IpzcsTuUE5r94Ss6iL42poDsWmuozHf2j1LD4rVyuZjs0LRkzUFyxanz92yGkt5yhCJKbNQUpGRAJN2AVkyQfAai_gsX_gdr32o9O6dm6dhZjKd7djZ19iIPpeOsYoVkhBwxuLTj2gvknL-f0H3d7BczUCKTiBe7uN34jSYKdW3E_BS8OW4y5AY4Azmie0sHFiifnV1YtaK4bOIF29_iljdtAQHeWgQfmRTb0v9uk1ZMipsYwVOEV-KOnS2n6CwJXuSQowCGhv24yMhmlMu9kWjC0ihCdqJZjqrtISZAw
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-laura-eliise
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-laura-eliise auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-laura-eliise apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-laura-eliise`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-laura-eliise describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-laura-eliise delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-laura-eliise delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-laura-eliise apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-laura-eliise get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-laura-eliise exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-laura-eliise exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-laura-eliise logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-laura-eliise logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-laura-eliise get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-laura-eliise delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-laura-eliise apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-laura-eliise get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-laura-eliise exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-laura-eliise apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-laura-eliise.k8s.vrfgstest.org/
  - host: "vrf-laura-eliise.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-laura-eliise.k8s.vrfgstest.org/](https://vrf-laura-eliise.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-laura-eliise get ingress
