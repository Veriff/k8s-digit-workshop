...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/test-canary/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..j5nwZK_gC2RknYCINXp0iouL0WbkLtr_BhQhdpn5C9X2y53wj8BEf_kbkyPWFnKlicepJKeKP3kpwQx7Y6JUaLXLVP59idzyiv9cbqX7bc85C2wHlkk1J7hhQFassg0JI9Fbo5TTOdgEQd3VSE2c5DH-Pen0YoikC1Wc82yjDDMpProRzScgvdAfUFoWDv6CUIzHy_T4bwnK511fx_dh0fNTn_0dgSco2C-yT-pAeE4jSas1WNc3tGaRqn-ZwuBfLCGOwipW8o7nvYwNktMNeczVYdW3aTKocBwPoI3821BXlpHMbfqWHRzY0EQ10nR2xtinLuuNHhkqSHrRJQOxnQ
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n test-canary auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n test-canary apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=test-canary`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=test-canary get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=test-canary get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=test-canary describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=test-canary delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=test-canary delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n test-canary apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=test-canary get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=test-canary get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=test-canary get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=test-canary get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=test-canary get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=test-canary exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=test-canary exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=test-canary logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=test-canary logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=test-canary get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=test-canary delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n test-canary apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=test-canary get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=test-canary exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n test-canary apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://test-canary.digit.vrfgstest.org/
  - host: "test-canary.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://test-canary.digit.vrfgstest.org/](https://test-canary.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=test-canary get ingress
