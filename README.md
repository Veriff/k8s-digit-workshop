...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-friedballerina/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..l7a5Z2dFruuhEyXn0Jg9-rsctCKQbgDR2TkP9VzfEA-6DIaUsm40nCggQRLfg_hWjH0SZytpJMeviW_VqbtFy_pbg97v4O8Mu-RTibOb6Om6rk4mx0Fc_i-VcuzlVe0aGo7g0RWXigC6w4hMxWpWcO18Him6beWr1l6FaAmPK4iTVgHQ1muMdQJrAmA1Mo1DPWBmixgnlvN9EQcyZmbAZdJ2p6ZgiDBlZHYjA1i5fRXAeHObLupnXjlunSX73DOE2epkAf9zHH8ixDVd5tVzy2C2vZJzsIFXIOpaa5Nd4X32E7QT4svFJ57TTi3ufkr-2X1J-bp-vcJWCgWsQvfMbg
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-friedballerina
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-friedballerina auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-friedballerina
```
...
> `kubectl --context=k8s-workshop -n vrf-friedballerina apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-friedballerina`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-friedballerina get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-friedballerina get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-friedballerina describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-friedballerina delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-friedballerina delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-friedballerina apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-friedballerina get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-friedballerina get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-friedballerina get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-friedballerina get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-friedballerina get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-friedballerina exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-friedballerina exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-friedballerina logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-friedballerina logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-friedballerina get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-friedballerina delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-friedballerina apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-friedballerina get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-friedballerina exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-friedballerina apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-friedballerina.k8s.vrfgstest.org/
  - host: "vrf-friedballerina.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-friedballerina.k8s.vrfgstest.org/](https://vrf-friedballerina.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-friedballerina get ingress
