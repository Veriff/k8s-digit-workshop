...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-erkki-tikk/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..QzMd6ppKtzy0Ud6rafd4-Q_ElIDfAxlmJxGclON_8eCykE2PptyuluiKmzhC10QQNorPy819gubgdCNPmawlV3qD8FpOz8NVR4DDnnwDvwCT57JHPfr5q20wh1iYqmZg-iBGWtdYH3N-V1KCdZlzc36NV1TrxtJy2Qk2L_LfCKmxvqy5qcjU2j1KvAToNZk4e73pk9zG0R3hnVxK1OsBpCAW66_k77MCoQMGFgBZObfxmX3MJACR_auYFkHEVDeoJPRLZIHV0U94SfEqELjfll7p6s7a8WieCANR2ggjh7X5AFqxPCfh_ZrVn8CVyZYA-4sMUMtPrrUZcXEH21AbLg
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-erkki-tikk
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-erkki-tikk auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-erkki-tikk apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-erkki-tikk`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-erkki-tikk describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-erkki-tikk delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-erkki-tikk delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-erkki-tikk apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-erkki-tikk exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-erkki-tikk exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-erkki-tikk logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-erkki-tikk logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-erkki-tikk delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-erkki-tikk apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-erkki-tikk exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-erkki-tikk apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-erkki-tikk.k8s.vrfgstest.org/
  - host: "vrf-erkki-tikk.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-erkki-tikk.k8s.vrfgstest.org/](https://vrf-erkki-tikk.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-erkki-tikk get ingress
