...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-wuff-wuff/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..H-TL52KWIfH_vFt3zc9mL438rKCWFSB5LL_wsZg7exRA8d0CaRjXlxJEWr9LfGDlMRNceym4IAjHifsaM_DtVCwKURTuBXgYDCLcXReiKGMDk-_p_FB2Q5zSLpQFtLGJvW5XqiUpP2-WSML0ZlVZADEoNFJWEo5WoHEbfGyGbUa36mkczJ00rAPd7vRzW67U8C04yn4aJpdA5ioLFyYn53DGcMz2ztyqmwNkoJqe-vKtE9XTBdvaJFyMy66PCPM3Rt52i44DyHdqfm_wEBO7OqtPe28zPd7syVweDXJzhSSRL3YWXqzxjOQ0__deMJ2hlmnWA6C46uG0kesyxhlrlA
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-wuff-wuff
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-wuff-wuff auth can-i list pods
# should return yes
```
...
```sh
git clone git@github.com:Veriff/k8s-digit-workshop.git
# move to folder where code was checked out
cd k8s-digit-workshop
# use your branch
git checkout k8s-workshop-2022/vrf-wuff-wuff
```
...
> `kubectl --context=k8s-workshop -n vrf-wuff-wuff apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-wuff-wuff`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-wuff-wuff describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-wuff-wuff delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-wuff-wuff delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-wuff-wuff apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-wuff-wuff exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-wuff-wuff exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-wuff-wuff logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-wuff-wuff logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-wuff-wuff delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-wuff-wuff apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-wuff-wuff exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-wuff-wuff apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-wuff-wuff.k8s.vrfgstest.org/
  - host: "vrf-wuff-wuff.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-wuff-wuff.k8s.vrfgstest.org/](https://vrf-wuff-wuff.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-wuff-wuff get ingress
