...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-erikh/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..Orf8ekhhWObDebhSRr1jWUCb1sNLmdnET9GggpmSqiqVhMG3QWiZZY4GL-_R9Q9SpXpzUNzmr_q-FgayEBrBltiO4kswQBAKmsSISoF0aiOR_h-OGvTDbnaD_kgFKb5Rg2hSPLafigybURMM1qY7xhD2neh7UB2ywkR8QOCkbcIZZ4ZEqZnyeAbW4fsIR8LWbE4htTCCuQS4fD9j9V1UAi8RPRVrAk6MAImN9hPbfqDS6WyMmRcnCOalgLNRpJKWSdbPf80pc1NHe1PEg4-Y_tggY4dSeZJXuOcFApaV_jkE-WXNV0sGoUWAKHqbBrhkuQnpZlVjH0aP1Np5c1HXgA
contexts:
...
```sh
kubectl --context=k8s-workshop -n vrf-erikh auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-erikh apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-erikh`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-erikh get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-erikh get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-erikh describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-erikh delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-erikh delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-erikh apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-erikh get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-erikh get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-erikh get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-erikh get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-erikh get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-erikh exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-erikh exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-erikh logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-erikh logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-erikh get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-erikh delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-erikh apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-erikh get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-erikh exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-erikh apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-erikh.k8s.vrfgstest.org/
  - host: "vrf-erikh.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-erikh.k8s.vrfgstest.org/](https://vrf-erikh.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-erikh get ingress
