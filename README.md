...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/asdasada/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..oPQBYwAQnWBwF0sFTgjhweaBQBgtCexL9zYIpYUUZls3JZC4LXMdHEGFA0WxzoPhzeO2mgMjcZQNWyIHIdotSbDPNbpm3evUzZOwztpeqIqAHSca9w33cHAJGzGTihhaX5755e8nTj-3ronP8SZV3eI5evCPTDQuaAUksE5ZhSVuaYn7xPLMsqAqS25DzdwTdNV93bvF8BwyPRbbwGe-pBzUzcW7ntjnpPD7XBYGXL-OiqyywkPBoqPQUekZbKdnrcliO6qyog7L61Btf-U7VbgbTmswZ9MH6Fa_qF3lSbVQo4mLSMB59a3c1hRavorNRXgCjWQN73jgB1TkjD3Dxw
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n asdasada auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n asdasada apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=asdasada`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=asdasada get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=asdasada get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=asdasada describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=asdasada delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=asdasada delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n asdasada apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=asdasada get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=asdasada get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=asdasada get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=asdasada get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=asdasada get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=asdasada exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=asdasada exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=asdasada logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=asdasada logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=asdasada get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=asdasada delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n asdasada apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=asdasada get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=asdasada exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n asdasada apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://asdasada.digit.vrfgstest.org/
  - host: "asdasada.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://asdasada.digit.vrfgstest.org/](https://asdasada.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=asdasada get ingress
