...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/roman-shandurenko/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..Qd5UnKufGTTDTjDsJyE8-BJ2uBxSLlKgGuyVpZ087-YjPcu4m0451VzHCiNeXeU9RQbYxsYAj9thlBVweq_ri6J0wuf8mfLQs3JvnGXdJw1I1KiQdtRq6RNhtwfQY7p6JBIWfeHqjbfOXu8K_SShvMooN_N_1UWFyYtc47PrtfqIJXniNLXyT1sWa_VSE4oi66PNqV7GEJukfNJgmRaq5MB_Uu8-eFQeOqEH7xOzgnJVbrjvHYvMGBIF5XzJB69aiMM2tO1t5impocBEOy8rxZirbq8ydo6ZaSksdFNh0OWDxqGRT8RiZu21xJYWvp8rfcAbVtsn33HnuVVXgOb_4Q
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n roman-shandurenko auth can-i list pods
# should return yes
```
...
# use your branch
git checkout roman-shandurenko
```
...
> `kubectl --context=k8s-digit-workshop -n roman-shandurenko apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=roman-shandurenko`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=roman-shandurenko describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=roman-shandurenko delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=roman-shandurenko delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n roman-shandurenko apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=roman-shandurenko exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=roman-shandurenko exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=roman-shandurenko logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=roman-shandurenko logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=roman-shandurenko delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n roman-shandurenko apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=roman-shandurenko exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n roman-shandurenko apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://roman-shandurenko.digit.vrfgstest.org/
  - host: "roman-shandurenko.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://roman-shandurenko.digit.vrfgstest.org/](https://roman-shandurenko.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=roman-shandurenko get ingress
