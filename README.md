...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-tseppern/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..Hucapzo1vFuPly4iz9JC2RSg6vBA7gyZDWUia6VgdNGBRrMpHUSSDKNcb-ts5gEkf30R40T3Uq8j_BqC8Gc2y4CVJjFNre_l8BOJ_LKjWEden3tXJkxDtI6qEjPvGp9bQ3PIs7hAMU2tDGAQ2TODYrD7SCGVZXZ5239TaB2s-YbQMDX2pBep9_7s-YBD3VrzET6_9UotqnAJsifSUI-bwOWglukTHfK6C6sAA4g99Ja6TjK4TQUUxf_228eaDJ_dNVlVRAKvQewgiY3nZAofp-WrLIo6fatStFjUL7DGv75ANVDMfWuHg5AE1tolypIsFJst7VrpZdFdhVVmjm39Ig
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-tseppern
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-tseppern auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-tseppern apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-tseppern`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-tseppern get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-tseppern get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-tseppern describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-tseppern delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-tseppern delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-tseppern apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-tseppern get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-tseppern get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-tseppern get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-tseppern get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-tseppern get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-tseppern exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-tseppern exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-tseppern logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-tseppern logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-tseppern get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-tseppern delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-tseppern apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-tseppern get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-tseppern exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-tseppern apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-tseppern.k8s.vrfgstest.org/
  - host: "vrf-tseppern.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-tseppern.k8s.vrfgstest.org/](https://vrf-tseppern.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-tseppern get ingress
