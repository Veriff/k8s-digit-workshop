...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-inquisitivetable/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..w5DY_dQWMoVKbCsrULQfqa5G4Ddu_sI7VMt0bG7fkQ4ftN3rul23x-FJqzisi_O9ctdYI9CvA1ke9ABd4JG7pSfq2YkhegphqjuboIVP0Mwu4aWRs0_4uRNd_kyCoDOW3naRrhKI_i8J21-m8fem70q-ExkBskVNlhmxH1T6R2fgA65lsbh1ljMOUIGx528GoJAvMy1tTf8WmDlCT6LW93sivUWuwQr2Ln5oqkaell_jNNe6B4BUBPqLKNoSEC9s7e99siaG6S5dU7QfgUWvw6eBKbBeTr3d7SStYvOScibrNdUXsVB2l-sqIIxm19s-0T2HP7BT5_V8th6rcU_e6Q
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-inquisitivetable
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-inquisitivetable auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-inquisitivetable
```
...
> `kubectl --context=k8s-workshop -n vrf-inquisitivetable apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-inquisitivetable`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-inquisitivetable describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-inquisitivetable delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-inquisitivetable delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-inquisitivetable apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-inquisitivetable exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-inquisitivetable exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-inquisitivetable logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-inquisitivetable logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-inquisitivetable delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-inquisitivetable apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-inquisitivetable exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-inquisitivetable apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-inquisitivetable.k8s.vrfgstest.org/
  - host: "vrf-inquisitivetable.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-inquisitivetable.k8s.vrfgstest.org/](https://vrf-inquisitivetable.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-inquisitivetable get ingress
