...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-oliverp/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..2VCZVkhDfsJ274xnNSZQBMQQe1C4M3jAr2Y4oAKTQePSmQpBuiWG4cVq-8pflVHpKhKX4Tmxyp5mNu1ngnbZzSa6GWBuJ_74m8Zfa3VOMRw7FfAyYpHzGEc85Htj34Cf_HocIgIzkTv4vKXwvDz0sRjPBpiIx6fFX7aBQND7-G6_qQiLXqIXxnuJJd5GDxuiWC64oNTBlgpeSE94OdFun68mMoDmTfKq9hmQMCsYoUfopQ0VVHWPbo1yliTazbN987HnFt5NpZUgKtBai_G76zNL8896GgkIiRbX71ZkG-D5mQtiaNLIcM3jXAoFbOapMDOJkpAvJubdzKoMddvl5A
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-oliverp
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-oliverp auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-oliverp apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-oliverp`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-oliverp get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-oliverp get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-oliverp describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-oliverp delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-oliverp delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-oliverp apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-oliverp get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-oliverp get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-oliverp get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-oliverp get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-oliverp get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-oliverp exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-oliverp exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-oliverp logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-oliverp logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-oliverp get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-oliverp delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-oliverp apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-oliverp get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-oliverp exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-oliverp apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-oliverp.k8s.vrfgstest.org/
  - host: "vrf-oliverp.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-oliverp.k8s.vrfgstest.org/](https://vrf-oliverp.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-oliverp get ingress
