...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-keiti-nurmbek/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..MvS474VA0xJf4ImMHn2GBzDJy6Mu5riImz86aLfNP6vnSV7mW61DImMfRLgR7QqEdGJ-tJB5_a_cy_mnpr4IRFei2zAOvI4huLeAhJeGamOgpxnrMb-7T2q3OrcAjmpLluwUzjJmkkAJinVbyqUo5ZkeNxcJ_IzFRqdlJF-BY-Bgvj0iiJfu8RnRF1_q0pJ_shbF88KA38DlaDqVyi8QK1GDFRXAxC5ZOz30C7tpEps4GahPHYCHmWHoUQ9QcI_xOymJkd8UG4_TB0k3Ddm-KCq6jRbK0NaV_r4TQla-boZdEr_Em2Sm4r84YqlNHMZiGMqxRDd6owBJ4VsS4PVbSQ
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-keiti-nurmbek
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-keiti-nurmbek auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-keiti-nurmbek
```
...
> `kubectl --context=k8s-workshop -n vrf-keiti-nurmbek apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-keiti-nurmbek`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-keiti-nurmbek apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-keiti-nurmbek apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-keiti-nurmbek apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-keiti-nurmbek.k8s.vrfgstest.org/
  - host: "vrf-keiti-nurmbek.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-keiti-nurmbek.k8s.vrfgstest.org/](https://vrf-keiti-nurmbek.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-keiti-nurmbek get ingress
