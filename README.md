...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/asdasdas/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..IK27W1PHKRuCwtU6TlVV0Of76vF21N7uIfnrWGh_EmkR4g2LLotPy8-n4PWyNhlpWM2s8eAKs3goQBC4pNY6OO9P1pPOChhPnnCiz9h8QvC-k2nO46S-laCDxW9zSIdgA6N4my8OeBjLCweDpxZ-BrMROMMN6LiaNoHsAUE1-clQdisIL91pPWQK8LKEZxO4X-ACGk-V9mVav9crS0XFgLTmqj5g9-bWu4mlyRs0doGPH_jsDrvEiCwAxkciKSEVENDXFLZUKYKe9u1PRs7aXkd2zjFtaE1I2TTni4c3b1x8TrMhjTdaoGfM0rYJVSoKvSJQG0Yh50NuOde5IM2AmA
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n asdasdas auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n asdasdas apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=asdasdas`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=asdasdas get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=asdasdas get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=asdasdas describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=asdasdas delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=asdasdas delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n asdasdas apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=asdasdas get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=asdasdas get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=asdasdas get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=asdasdas get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=asdasdas get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=asdasdas exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=asdasdas exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=asdasdas logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=asdasdas logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=asdasdas get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=asdasdas delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n asdasdas apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=asdasdas get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=asdasdas exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n asdasdas apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://asdasdas.digit.vrfgstest.org/
  - host: "asdasdas.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://asdasdas.digit.vrfgstest.org/](https://asdasdas.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=asdasdas get ingress
