...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/siim-tiilen/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..J-aIH2qAxmMSZMcFUikVmkl_2hA4gYSDEKtbFGwy3o3yIL1Rtf0xRJCsyhfC_zri9MjV8-EAxPoh2bVvwFqZSAYRGkh3oK7iYaC2Ttks1htFLGEPcmI0mL2jW1fj8alWbPzgKTLznzB9aQwgIe6Dh3g0PaztRZG8-IJhjzoqDfNDznVjs2tVQ1LYI4hDQAxmR3kxSNSnOenyH9eM7wyStaynhTFnPIi1IeWQ_u-sJFGRYztrvoCFqVuE6b3_mxDVa-t-jcpqebw7fpkvK_RPaTK_TsEMM76Q5gRjHW0rv1U1fTuGygpk2IxqQXpSkm3Wr-EtJl1CVrLi-5SY_L2vgQ
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n siim-tiilen auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-digit-workshop -n siim-tiilen apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=siim-tiilen`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=siim-tiilen describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=siim-tiilen delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=siim-tiilen delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n siim-tiilen apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=siim-tiilen get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=siim-tiilen exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=siim-tiilen exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=siim-tiilen logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=siim-tiilen logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=siim-tiilen get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=siim-tiilen delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n siim-tiilen apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=siim-tiilen get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=siim-tiilen exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n siim-tiilen apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://siim-tiilen.digit.vrfgstest.org/
  - host: "siim-tiilen.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://siim-tiilen.digit.vrfgstest.org/](https://siim-tiilen.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=siim-tiilen get ingress
