...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-siim-tiilen/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..P5aV1in7zHPCMz3_i1GeOgrc_aXwDU8UNWUayhPUq-Og-ApHoz5vLEZ8SyzouSVB793N-gdSAH6k1SV4rmX5FYIAEV4sSEwOed-AGVZsp77GY2yCYIYgCL4gIE4Uql_buQR1osIqCkgMJMXAP95ORH8frqM9j-jSrjpvNYz28ENNKXfK96_W16EU0z_7vJs756gsRphMq-AwzeUddpQdK0bHaeLBkT1HSR-6E8O2yTFfZVAbXyDdjhzsL0Cf3-xWVNOGDKWA3lsIngSPZwUEZ3an_G9e_TC-d1fS5hMzyBzCqcgy2gmOvJgFnHO4HVfSVLkU1wYzOovUIxsFnYIewg
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-siim-tiilen
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-siim-tiilen auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-siim-tiilen apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-siim-tiilen`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-siim-tiilen describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-siim-tiilen delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-siim-tiilen delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-siim-tiilen apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-siim-tiilen exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-siim-tiilen exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-siim-tiilen logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-siim-tiilen logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-siim-tiilen delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-siim-tiilen apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-siim-tiilen exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-siim-tiilen apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-siim-tiilen.k8s.vrfgstest.org/
  - host: "vrf-siim-tiilen.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-siim-tiilen.k8s.vrfgstest.org/](https://vrf-siim-tiilen.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-siim-tiilen get ingress
