...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-robert-reimann/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..3aN7i8r_q7J-pJxyfsB0TWtCsNaZ4U6E8MZE1Q3Wic8gJI5nLLmUAx2DnSks2gz0GyIV5GRaIBv_DUj8cdsw-FA7Crb7BCDXxsucfz09JH_E1bYfEskLWZa5qvIGVJ561N2xzJP9eTFX-3GHAo6BRRR3CoGp_92UyLzZ1vaMHIHM4cX6umKnlnxfCiqp7XRSbGehqHBRRU43K_rv2uYBTS5ip_jL99NonZtiCFbcFrWF60fnCfISZ6TNZ3ggWG5twUCC_rDW1isgxLNxmAAV31ajig2nrzruLQTvE9NF_oQufGHR5FIJ93lOh2j5AzR6Q2EGnTzZqPdyp4Ql9Ct8aA
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-robert-reimann
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-robert-reimann auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-robert-reimann
```
...
> `kubectl --context=k8s-workshop -n vrf-robert-reimann apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-robert-reimann`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-robert-reimann describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-robert-reimann delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-robert-reimann delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-robert-reimann apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-robert-reimann get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-robert-reimann exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-robert-reimann exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-robert-reimann logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-robert-reimann logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-robert-reimann get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-robert-reimann delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-robert-reimann apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-robert-reimann get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-robert-reimann exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-robert-reimann apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-robert-reimann.k8s.vrfgstest.org/
  - host: "vrf-robert-reimann.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-robert-reimann.k8s.vrfgstest.org/](https://vrf-robert-reimann.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-robert-reimann get ingress
