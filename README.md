...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/testime-465/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..UEN0l7WNnJKsCUwisxrZar65cK6WagQEcJiuTSaXh8cCa7rJ0OKlCWp-RpSdJHU71cyBGvm1q4hrsa-WfP1dyavlyVhu2_bawtKmNbY9JUE7KDDFpA7r_bBPC4QyQU49djhiQKJGSW6zQtA_UzL-oBi4PYlrEwwzqdYFBYJnOtZQjr174fj32dmE1hpve_GVKGqgZEtDHKLjwkbQ8-I80Aq9vetYOjJVl_O7rXP56Ptya_LJAeZYDqlLWzU-1p1UAZuaQUQJFrhdpLikaecjDqhH4L4IUvhLLzXeLrZwWdUPAqFGXGAKnfj5QFMLQQR2Q41lKnh9gRCzdL29ESNRmQ
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n testime-465 auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-digit-workshop -n testime-465 apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=testime-465`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=testime-465 get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=testime-465 get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=testime-465 describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=testime-465 delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=testime-465 delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n testime-465 apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=testime-465 get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=testime-465 get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=testime-465 get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=testime-465 get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=testime-465 get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=testime-465 exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=testime-465 exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=testime-465 logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=testime-465 logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=testime-465 get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=testime-465 delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n testime-465 apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=testime-465 get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=testime-465 exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n testime-465 apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://testime-465.digit.vrfgstest.org/
  - host: "testime-465.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://testime-465.digit.vrfgstest.org/](https://testime-465.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=testime-465 get ingress
