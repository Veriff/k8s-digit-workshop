...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/aivof/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..Z1XKMWFAlYO6OqiOcHm8w5-bHui_mmbrCWPhNxohFl9_fvLQa33M0fP1uhmIPgZZSdo9F1K-9aQcW-ho_sGRryZU04exWkpRUdOQmMwIQPM-eb0RaFA-GHMl-jMJwSt0yTkKPYrKB9XpMN-rb10z8hdiWHA5gYAfNByale_2ODMDIzHnv-j1oyaARoXv7TwYGYL95ex52XB7dVdu7hUnLCrf9PF_rt-R_iSFbrK8kQlAviB3CKk4fhvvfZxYMyU7KNMLV9L_o6OcB_5tR06Wv8zu0-9w28reNBMFe-noElfmIrQ7ZkHKfRi0jRdubtn7mB4_8B1XZGb9jbsudPy77Q
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n aivof auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n aivof apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=aivof`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=aivof get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=aivof get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=aivof describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=aivof delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=aivof delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n aivof apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=aivof get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=aivof get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=aivof get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=aivof get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=aivof get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=aivof exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=aivof exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=aivof logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=aivof logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=aivof get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=aivof delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n aivof apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=aivof get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=aivof exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n aivof apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://aivof.digit.vrfgstest.org/
  - host: "aivof.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://aivof.digit.vrfgstest.org/](https://aivof.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=aivof get ingress
