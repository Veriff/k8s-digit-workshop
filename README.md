...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-mihkel-leesment/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..kLxdrpuqD-XXLWWat5AhYvQ79Id1c_YsuSE16LQibNgsgUzch-3i1VCH2mWnWylp8V8pgO6c_nff7zD7DEbqcqt2XcCaVjLYvXBi-LszRqMdYUjlDYgEzPEKGx6xhFm2bxx4s8u6sU4KBkemS8m4rLf1-lErtgqW3DR0Pli8e6LAN60_4a4XAhLI4JIGQB86tz87KQmHoo-79rc0812xNIGfJ_tOix2zgk5ZpqV7FpOxOuL6N0A7Ch36Ej7RkTEgmltyZfaF1Q8WREz3CF-acSRxjhP7yK49t_YG2KGP9iLthvsU1dsNqAyD44CkUZmubOXfy363syHEDqea61J4wg
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-mihkel-leesment
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-mihkel-leesment auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-mihkel-leesment
```
...
> `kubectl --context=k8s-workshop -n vrf-mihkel-leesment apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-mihkel-leesment`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-mihkel-leesment apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-mihkel-leesment apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-mihkel-leesment apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-mihkel-leesment.k8s.vrfgstest.org/
  - host: "vrf-mihkel-leesment.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-mihkel-leesment.k8s.vrfgstest.org/](https://vrf-mihkel-leesment.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-mihkel-leesment get ingress
