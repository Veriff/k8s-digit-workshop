...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-casdhsf/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..KRWtbgYOLQhRC4cMuGrLxgT2JqMfHSew1GwyIM6xzJfBlxhqsNiPUSbT77OynbjaOzqb33xzUfhQKWp8FpivMWq5_y0htzVVBx5_-sPSqvoQ15xBFtnDCA-Xo_sqj3hzp2sWtGx57-R_PT6_JODIvj8Eb2JWzoc1e72EQeR4jL3EQFaREp6TRQz8KW1se1RpTDlf4BAj07SAHwCV-wdWuThe9mbclv0qyUV7KqAQUCB7KkydUeuqpQr3oIsAaTVhVHQhbuGd5T7BCjq-nXNEXmuXy6BjZg0UNT2fwgGtln69priXXkjvc3rC0HvJ4LU_QazV1dlhiH82ukf5-oex4Q
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-casdhsf
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-casdhsf auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-casdhsf apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-casdhsf`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-casdhsf get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-casdhsf get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-casdhsf describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-casdhsf delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-casdhsf delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-casdhsf apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-casdhsf get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-casdhsf get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-casdhsf get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-casdhsf get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-casdhsf get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-casdhsf exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-casdhsf exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-casdhsf logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-casdhsf logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-casdhsf get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-casdhsf delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-casdhsf apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-casdhsf get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-casdhsf exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-casdhsf apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-casdhsf.k8s.vrfgstest.org/
  - host: "vrf-casdhsf.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-casdhsf.k8s.vrfgstest.org/](https://vrf-casdhsf.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-casdhsf get ingress
