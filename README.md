...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/chika/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..WicrtE3w7QrVXsNPzgCOCjxb13ynbM46K70207m4IdoooWvfyY7sSIVPZNUQvY_Fr-_AfZIqKB1Nko2MLzPl58VwgnUuD6dKYrBHdJuAO97moZoMU5i9zdJH4Vs9CCD_Hxvj4kkZ-BOHbGrVvSUMfqZcs25C-PNYImFHOeX-7jJImCr4_O65DQWxACM9yM3bucMOwMOx-Sqt0RtoPG8Nrn5uQVTEetvkXrMTdI167mxXLhTaf5idzLp9uCUd_j2yyxQ9LtlDpgWz5tsWcrcOqR0PWKrOdIUbP3UTnQpDk0FeQBI8hSCAAWISZhaXNcpzkUcVEKBJARzvEI-fjhppKg
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n chika auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-digit-workshop -n chika apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=chika`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=chika get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=chika get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=chika describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=chika delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=chika delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n chika apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=chika get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=chika get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=chika get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=chika get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=chika get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=chika exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=chika exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=chika logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=chika logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=chika get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=chika delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n chika apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=chika get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=chika exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n chika apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://chika.digit.vrfgstest.org/
  - host: "chika.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://chika.digit.vrfgstest.org/](https://chika.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=chika get ingress
