...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/siim-test-6/kubectl-config)). 
...
- context:
    ...
    namespace: siim-test-6
  name: k8s-digit-workshop
users:
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..OfhkQe74hpDfSzp090NLj_fXZHUVfJ18u-_qYebJOevUJJ3nGGxFnHTF1Bh6fZVpq4m9v1ocQ0Traf6i13xHfdvv7fRA6s72qGMsn81ExtsgDSLQQiYCRNcYsqumUeelNECVNrAWvTJgzL09ZeFq8fTkeX6iCo1Yahc-gjb8UkAGLLgzCl3jUiyyfbQTa34oYcOIF52i8aZjBAetUOnsA5Kqh3kWhpPSOGPCw5KvyMI2eAnFb5d7dZR61k7VAVEx_YycBvlOR81lWwVwmrKwHKyAUZWqQ92i7lWh821SVHXduPIjS6Snj0inR-qz3AjYHrABwkScoB-NdpPd_Lh3Pg
...
```sh
kubectl --context=k8s-digit-workshop -n siim-test-6 auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n siim-test-6 apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=siim-test-6`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=siim-test-6 describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=siim-test-6 delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=siim-test-6 delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n siim-test-6 apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=siim-test-6 get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=siim-test-6 exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=siim-test-6 exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=siim-test-6 logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=siim-test-6 logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=siim-test-6 get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=siim-test-6 delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n siim-test-6 apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=siim-test-6 get services
...
> `kubectl --context=k8s-digit-workshop -n siim-test-6 apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://siim-test-6.digit.vrfgstest.org/
  - host: "siim-test-6.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
```sh
# get list of services
kubectl --context=k8s-digit-workshop -n=siim-test-6 get ingress
```