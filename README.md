...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-olari/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..i56H9WNX3hENtzmEe8E02XBgeZzF6GITZ5BGAqJQi8lih8dBDMSXk9jZIBaYjoAJ5Bz0ZDHJLDR-kiqCc9oJt4KbFeczBSXjm8WmTZ3Y0zQXz9jbBwyeUdZNHsqIpFeDTXGx3xQmLmj2JlwIMNFmYfFlI00Y7FCxzgc-etOXCntO67jR8zYQuSb-WXqW1-NV5TcVxs8-c38j5sIqeOUXW884y-xnB0pjq49nukjzkHiRkhmVsxrlD0VW4Y5Upwry2g0qZkYcnubVbCFCvYMifC0OqZM6GsznAhJW38I4jzDgxMOnTxo0OZdGd6QcoU4Ap9llQ1WyOP8JbhFWJWYoHw
contexts:
...
```sh
kubectl --context=k8s-workshop -n vrf-olari auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-olari apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-olari`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-olari get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-olari get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-olari describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-olari delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-olari delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-olari apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-olari get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-olari get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-olari get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-olari get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-olari get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-olari exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-olari exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-olari logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-olari logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-olari get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-olari delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-olari apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-olari get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-olari exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-olari apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-olari.k8s.vrfgstest.org/
  - host: "vrf-olari.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-olari.k8s.vrfgstest.org/](https://vrf-olari.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-olari get ingress
