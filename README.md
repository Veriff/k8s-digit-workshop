...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-fgvhbjnsfhsdh/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..LoI2QIN-WsBJou2nVIJrrY-Qyo4l-Q6tK57Z63SApNkttbw6iODy0PRJUHHZdosoEe-jgZfzzBIlAb2JCAP6UbSct5hxQtp1kR10isfxe2gSN3fUXLhUGlLiLRX9bsFJsKilMi0OrExrnAQ2Z59Q5L2ERjLulQ2DWtgZHWXL3gxuPdgAxIOrYtOsl48FkQ2140b9GDRmC9qKgCgRDq1iJcnawWPhawmTsyQmLz8xTif7_b-a43vRsPxhdjwVy_fJFVBk7PG7WtpDrDt9FkBCHfUkMNmFVQ0Ezcwsko_Zr3P2GQX2FtcBewFgTvhnTdWaqv9RxjcsJs9adsILYHxDFA
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-fgvhbjnsfhsdh
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-fgvhbjnsfhsdh auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-fgvhbjnsfhsdh
```
...
> `kubectl --context=k8s-workshop -n vrf-fgvhbjnsfhsdh apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-fgvhbjnsfhsdh`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-fgvhbjnsfhsdh apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-fgvhbjnsfhsdh apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-fgvhbjnsfhsdh apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-fgvhbjnsfhsdh.k8s.vrfgstest.org/
  - host: "vrf-fgvhbjnsfhsdh.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-fgvhbjnsfhsdh.k8s.vrfgstest.org/](https://vrf-fgvhbjnsfhsdh.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-fgvhbjnsfhsdh get ingress
