...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/siim-tetssst/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..pJfSwEumxlqU29bBKlmLSxB3co4_D5K-295rzXBPPqjc4lqtx1FyxiT4qj6x7f-NOZSFAPSba-chZ0mY0T1yc436RLiltz0zhxn3LM9Rok2R2kgswdeIWqUghnIdvgsCYg1C1AxJnS0Le-hhO5bvqT2H-fCl8bqlo3CewNTvEjODQvMXXQ4r9jf9rKhyODjkG8h5qMvH6qd92-wpO3Tui9_iJeh6tohWoyQ7BZrQFIdetDArpmA6aVEXWERR6sfBjAvs3UHnr0rkL3a_8kYUPz0irhrEyID63_yVD_Y2aO-WjKrv0DomLSEQU-otXIRmkfYcq9Eo0BKCMbACssslQA
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n siim-tetssst auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-digit-workshop -n siim-tetssst apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=siim-tetssst`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=siim-tetssst describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=siim-tetssst delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=siim-tetssst delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n siim-tetssst apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=siim-tetssst get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=siim-tetssst exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=siim-tetssst exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=siim-tetssst logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=siim-tetssst logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-digit-workshop -n=siim-tetssst get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=siim-tetssst delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n siim-tetssst apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=siim-tetssst get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=siim-tetssst exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n siim-tetssst apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://siim-tetssst.digit.vrfgstest.org/
  - host: "siim-tetssst.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://siim-tetssst.digit.vrfgstest.org/](https://siim-tetssst.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=siim-tetssst get ingress
