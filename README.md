...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/asffafas/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..D-JXyRgdg8ymJjpGvJQ1hd4WrFph4478YH0oNJ4nHQ4rrch72M_IRAauGx-1riJlp1uWnuqkxHpZ4mKTq-k1fCubvoGoky_1HCbX5rq2SzS1lJB7wCPs98shew7uP8XSorfJcWJH8hOHKJm59Uz9eBBt8lclVNBj9tYvsxtfq1DrsfUXWd36E0EMivxONSz_lRo6VJwmgD8zg5NeT9XMZnXmXDZB6MZNVP_kQ_CdAdWuEOh-SVeRsfZ2esVHBGef33L5s4VGLZ55KboZoZVpbynRP55ss6CulEQPbj_s0euIu5-6e3zsSIOt2yzV-GaxhJip4zoXv4IJADFz7WZBOQ
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n asffafas auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n asffafas apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=asffafas`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=asffafas get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=asffafas get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=asffafas describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=asffafas delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=asffafas delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n asffafas apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=asffafas get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=asffafas get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=asffafas get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=asffafas get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=asffafas get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=asffafas exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=asffafas exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=asffafas logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=asffafas logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=asffafas get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=asffafas delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n asffafas apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=asffafas get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=asffafas exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n asffafas apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://asffafas.digit.vrfgstest.org/
  - host: "asffafas.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://asffafas.digit.vrfgstest.org/](https://asffafas.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=asffafas get ingress
