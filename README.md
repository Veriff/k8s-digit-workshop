...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-kasparcluster/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..o3hZlvK6l3fPOBsxlL-sPXEMJPxwcub6A4Ckbirk6Sp2CHvBHA7uorJsOPh3SUUfUsphxpQCEvwBYHtucOJk78eSsIT8p8WRXxNi15YlVUiPJux40EjpYWBfFCvV6w2HmG1JgPebTHdWludQbmlGZCme3ML-EJr1itWvpEixCQK7xXmpWZaJileTZQLhSDUR5P8mx9hnZXnUrbyOuqP_w_TFd106N41skjV8vOswQue8oG5gZqJf5wT3UjaF02CLZiBKk3gNDgspjE5BwcsJGVZFflWGTqXVLRPnVM2ipGkl2-dTuu3t0Df_ORIKaN3KTkwlm4qKyRozV5IrMmwnow
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-kasparcluster
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-kasparcluster auth can-i list pods
# should return yes
```
...
# use your branch
git checkout vrf-kasparcluster
```
...
> `kubectl --context=k8s-workshop -n vrf-kasparcluster apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-kasparcluster`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-kasparcluster describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-kasparcluster delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-kasparcluster delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-kasparcluster apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-kasparcluster get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-kasparcluster exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-kasparcluster exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-kasparcluster logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-kasparcluster logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-kasparcluster get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-kasparcluster delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-kasparcluster apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-kasparcluster get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-kasparcluster exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-kasparcluster apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-kasparcluster.k8s.vrfgstest.org/
  - host: "vrf-kasparcluster.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-kasparcluster.k8s.vrfgstest.org/](https://vrf-kasparcluster.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-kasparcluster get ingress
