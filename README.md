...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/vrf-jarmos/kubectl-config)). 
...
- name: k8s-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6IjZSMHhwYm1LN3d2Sk1RUUJFaXhOQ2JqZEJocDZsc1JkYmZhM2czZUxhVm8ifQ..EkpSm0mGGfKjMYb3qrFqe4JR_IcjG3BgOKYM8iHWcaLPLDkwgYdeam-IUYBTSwzoyCLnd6x1CT4MwWh1yZ418blxh2xVlMtKS1Hh5tmlk73CD3Z116vwVaIgauIU6vNRdMBm8cAy4kFu4_iHJVxFsZqFyQLlYkR8c9olzyR0ZBtfmmkp7Gsfvc5cBgXDAuB-l80Xysk7AZKrJQogfAQjCx5w6gPwwbcgCTgIWqu5HF0N6gjIazRCp2-fDSndxk2YJesPVOSBAJvyR_S2IRKhXAIlcLnXWSvwYmx1mzTDkf1O2bLsYykk0rOJQxlkrEtxjPqJSOrH3X1NRGtWXCnRLQ
contexts:
# context is what you use - it describes for this cluster use this auth 
- context:
    cluster: k8s-workshop--cluster
    user: k8s-workshop--user
    namespace: vrf-jarmos
  name: k8s-workshop
...
```sh
kubectl --context=k8s-workshop -n vrf-jarmos auth can-i list pods
# should return yes
```
...
> `kubectl --context=k8s-workshop -n vrf-jarmos apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=vrf-jarmos`)
```sh
# list all pods in your namespace
kubectl --context=k8s-workshop -n=vrf-jarmos get pods -o wide
```
...
```sh
# show declaration
kubectl --context=k8s-workshop -n=vrf-jarmos get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-workshop -n=vrf-jarmos describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-workshop -n=vrf-jarmos delete -f examples/pod.yml

# or delete by name
kubectl --context=k8s-workshop -n=vrf-jarmos delete pod/pod-example
```
...
> `kubectl --context=k8s-workshop -n vrf-jarmos apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-workshop -n=vrf-jarmos get deployments

# get list of pods
kubectl --context=k8s-workshop -n=vrf-jarmos get pods -o wide

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-workshop -n=vrf-jarmos get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-workshop -n=vrf-jarmos get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-workshop -n=vrf-jarmos get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-workshop -n=vrf-jarmos exec -it deployment/deploy-example -- sh
kubectl --context=k8s-workshop -n=vrf-jarmos exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-workshop -n=vrf-jarmos logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-workshop -n=vrf-jarmos logs -f --tail=20 <POD_NAME>
...
# get list of pods
kubectl --context=k8s-workshop -n=vrf-jarmos get pods 
# delete one pod
kubectl --context=k8s-workshop -n=vrf-jarmos delete pod <POD_NAME>
...
> `kubectl --context=k8s-workshop -n vrf-jarmos apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-workshop -n=vrf-jarmos get services
...
```sh
# connnect to pod
kubectl --context=k8s-workshop -n=vrf-jarmos exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-workshop -n vrf-jarmos apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://vrf-jarmos.k8s.vrfgstest.org/
  - host: "vrf-jarmos.k8s.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://vrf-jarmos.k8s.vrfgstest.org/](https://vrf-jarmos.k8s.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-workshop -n=vrf-jarmos get ingress
