...
Once done add config to use our test Kubernetes cluster into `~/.kube/config` ([download](https://github.com/Veriff/k8s-workshop/raw/asdasfafsa/kubectl-config)). 
...
- name: k8s-digit-workshop--user
  user:
    token: eyJhbGciOiJSUzI1NiIsImtpZCI6InV6dkF1NUN6U0tiLUplTUQtbXl0WHFZRU5INWxrLWdwbEVLLV9rTlAzZUEifQ..F3SfB79iDKpHbukh6LnRGFg7uHe6iRcXd54Y1HXQwE69RcvyS0cC6OQHt6O2YCWSp-7g_D6o5a-xrqq6DrJ7L1Hu7qqVygcObgXk3PEajeGj_JBuM8uJQC0hx3nQo6sfDmFA0vNuxmZPLwBsOlRdxNW5yGEbyCRGxnVTQPlFV9XI_N--pBMgQyypfHhrzUsJCRqip4p6ZPcQtohgQvz6qdFtweYLs5vn7phR8SjdySDpOFoz9iGphjXjiJALjZqi3BveonBW9RC3D5lgbxb97qyKyZ9t--PP7z5iSUWyERCJakQXQYHExe9UhcCte2XRCk6D0N_7OuVNaY7QlHqDwg
contexts:
...
```sh
kubectl --context=k8s-digit-workshop -n asdasfafsa auth can-i list pods
```
...
> `kubectl --context=k8s-digit-workshop -n asdasfafsa apply -f examples/pod.yml`
```yaml
---
apiVersion: v1
kind: Pod
...
For testing, get list of pods in your namespace (`-n=asdasfafsa`)
```sh
# list all pods in your namespace
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pods
```
...
```sh
# show declaration
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pod/pod-example -o yaml

# describe pod
kubectl --context=k8s-digit-workshop -n=asdasfafsa describe pod/pod-example
```

If you want to delete the resource, use `kubectl delete`
```sh
# delete all resources from file
kubectl --context=k8s-digit-workshop -n=asdasfafsa delete -f examples/pod1.yml

# or delete by name
kubectl --context=k8s-digit-workshop -n=asdasfafsa delete pod/pod-example
```
...
> `kubectl --context=k8s-digit-workshop -n asdasfafsa apply -f examples/deployment.yml`
```yaml
---
apiVersion: apps/v1
kind: Deployment
...
kubectl --context=k8s-digit-workshop -n=asdasfafsa get deployments

# get list of pods
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pods

# get deploy-example pods (-l is label selector, you can give it to all resource calls)
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pods -l=app=deploy-example

# get all pods using shared purpose=training label selector
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pods -l=purpose=training

# show labels in output (-L)
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pods -L=purpose,app
...
# connect into pod
kubectl --context=k8s-digit-workshop -n=asdasfafsa exec -it deployment/deploy-example -- sh
kubectl --context=k8s-digit-workshop -n=asdasfafsa exec -it <POD_NAME> sh

# show logs from pod
kubectl --context=k8s-digit-workshop -n=asdasfafsa logs -f --tail=20 -l=app=deploy-example
kubectl --context=k8s-digit-workshop -n=asdasfafsa logs -f --tail=20 <POD_NAME>
...
kubectl --context=k8s-digit-workshop -n=asdasfafsa get pods 
# delete one pod
kubectl --context=k8s-digit-workshop -n=asdasfafsa delete pod <POD_NAME>
...
> `kubectl --context=k8s-digit-workshop -n asdasfafsa apply -f examples/service.yml`
```yaml
---
apiVersion: v1
kind: Service
...
kubectl --context=k8s-digit-workshop -n=asdasfafsa get services
...
```sh
# connnect to pod
kubectl --context=k8s-digit-workshop -n=asdasfafsa exec -it deployment/deploy-example -- bash
...
> `kubectl --context=k8s-digit-workshop -n asdasfafsa apply -f examples/ingress.yml`
```yaml
---
...
spec:
  ...
  rules:
  # https://asdasfafsa.digit.vrfgstest.org/
  - host: "asdasfafsa.digit.vrfgstest.org"
    http:
      paths:
      - pathType: Prefix
        path: "/"
...
And try to access your app using [https://asdasfafsa.digit.vrfgstest.org/](https://asdasfafsa.digit.vrfgstest.org/).

```sh
# get list of ingress
kubectl --context=k8s-digit-workshop -n=asdasfafsa get ingress
